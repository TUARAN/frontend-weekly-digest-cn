...
function firstValue(searchParams: SearchParamReader, key: string): string {
  return searchParams.get(key) ?? '';
}

function parseSharePayload(searchParams: SearchParamReader) {
...
export default function ShareTemplateClient() {
  const searchParams = useSearchParams();
  const { kind, title, summary, source, date, tier, href } = parseSharePayload(searchParams);
