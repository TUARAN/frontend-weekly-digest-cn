...
# [JS Bin down in 2026](https://remysharp.com/2026/02/02/js-bin-down-in-2026)
...
Earlier today [Jake](https://jakearchibald.com/) asked me: *what went wrong?*
...
![](https://remysharp.com/images/jsbin-status-chart.avif)In 2017 the entire server fell over and need to be replaced entirely. Then this were quiet-ish, then…we have the red wall
...
![](https://remysharp.com/images/cloud-watch-chart.avif)The 100mb peak *incoming* traffic was the real problem.
...
Props where due, putting CloudFlare in front of JS Bin was relatively easy. CloudFlare detected *most* of the domains and where they pointed (the important ones at least) and it was a matter of swapping over the name servers from pointing to AWS's Route 53 to CloudFlare's own name servers.

Close to midnight on 29-Jan, I started to see [jsbin.com](http://jsbin.com/) load in the browser.
...
This config was all well and good, but traffic was still coming in. This change would mean that nginx would have to process the traffic than ignoring (or "dropping") it entirely (*#foreshadowing*).

When I ran command below, it would list the connections, and I could *still* see non-CloudFlare requests being able to complete request (at this specific point in time, I was in a frantic cycle of "try a change - fail - worry a bit - ask LLM - repeat"):
...
It's a mess. Now I've got reports from people on github telling me they're still seeing *mostly* 520 errors, and finally I realise I can replicate by using a VPN because for some reason it was still working for me.

There's two problems at this point:

- Traffic still coming in that isn't through CloudFlare that's causing a drain on resources

- Real users coming through CloudFlare are, mostly, not getting the site
...
This was done on two fronts - just for *belt-and-braces* approach:

- `ufw` (or as I knew it "iptables") rules one the server to `DROP` or `ALLOW` traffic based on IP range

- AWS security policy to do the same.
...
With those in place, finally the server was breathing again. However, those pesky CloudFlare 520s were still preventing people from visiting [jsbin.com](http://jsbin.com/) (except me…somehow…).
...
![](https://remysharp.com/images/jsbin-htop-2026.avif)Just chilling out at an easy 4.6% CPU usage
...
![](https://remysharp.com/images/jsbin-hong-kong.avif)Hong Kong is so small in this map, you can't even see the deep blue of the chart

Since I took that screenshot, Hong Kong has 10 million requests in the last 24 hours.

Even though I don't think I'll ever really know what caused the amount of traffic that caused everything to fall over, my gut suspects scrapers for AI and LLMs just slurping up as much of the web it can. The only evidence I have *against* that theory is that the traffic didn't come from a single IP.
...
Published 2-Feb 2026 under #personal. [Edit this post](https://github.com/remy/remysharp.com/blob/main/public/blog/js-bin-down-in-2026.md)

          
            
👍 24 likes

            [![](https://remysharp.com/images/no-user.svg)](https://fedi.vale.rocks/users/vale)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/vale.rocks)[![](https://remysharp.com/images/no-user.svg)](https://fosstodon.org/@krinkle)[![](https://remysharp.com/images/no-user.svg)](https://fosstodon.org/@caillou)[![](https://remysharp.com/images/no-user.svg)](https://sfba.social/@studiochris)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/jrlarsen.bsky.social)[![](https://remysharp.com/images/no-user.svg)](https://kafeneio.social/@felixthehat)[![](https://remysharp.com/images/no-user.svg)](https://mastodon.social/@7studio)[![](https://remysharp.com/images/no-user.svg)](https://mastodontech.de/@jak2k)[![](https://remysharp.com/images/no-user.svg)](https://mastodon.social/@connorbaer)[![](https://remysharp.com/images/no-user.svg)](https://fediverse.zachleat.com/@zachleat)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/adrukh.bsky.social)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/deanpcmad.com)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/michaelspellacy.com)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/anatudor.bsky.social)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/edent.tel)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/bell.bz)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/andydavies.me)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/dom.happygiraffe.net)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/mastrojs.bsky.social)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/aussie.bsky.social)[![](https://remysharp.com/images/no-user.svg)](https://bsky.app/profile/andrewt.net)[![](https://remysharp.com/images/no-user.svg)](https://mastodon.social/@JosePedroDias)[![](https://remysharp.com/images/no-user.svg)](https://gts.efdlab.com/@eber)